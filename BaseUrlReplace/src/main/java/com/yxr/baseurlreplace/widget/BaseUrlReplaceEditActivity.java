package com.yxr.baseurlreplace.widget;
...
import com.yxr.baseurlreplace.R;
...
public class BaseUrlReplaceEditActivity extends AppCompatActivity {
    ...
    private void initData() {
        etBaseUrl.setText(BaseUrlReplaceSdk.getInstance().getBaseUrl());
    ...
    private void saveBaseUrl() {
        ...
        if (HttpUtil.isUrl(baseUrl)) {
            BaseUrlReplaceSdk.getInstance().updateDebugUrl(baseUrl);
            addHistory();
            toast("保存成功");
        } else {
            toast("这不是个正确的地址");
    ...
    private List<String> getHistories() {
        ...
        String json = SPUtil.getString(SP_URL_CHANGED_HISTORY);
        if (json != null) {
            try {
    ...
    private void updateHistory() {
        try {
            ...
            SPUtil.putString(SP_URL_CHANGED_HISTORY, jsonArray.toString());
        } catch (Throwable e) {
            e.printStackTrace();
        }
    }
